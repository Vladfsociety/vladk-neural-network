...
class Input(Layer):
    def initialize(self, device):
        return {
            "type": 'input',
            "a": torch.zeros((self.size, 1), device=device)
        }
...
class FullyConnected(Layer):
    ...
    def initialize(self, previous_layer, device):
        ...
        return {
            ...
            "learnable": True,
            "w": super()._init_weights((self.size, previous_layer_size), device),
            "b": super()._init_biases((self.size, 1), device),
            "z": torch.zeros((self.size, 1), device=self.__device),
            "activation_function": self.__activation,
            "a": torch.zeros((self.size, 1), device=self.__device),
        }
